...
const CardPatterns = (function () {
     ...
     * Trained on 55 XSPOC-classified wells.
     *
     * 5-class hybrid approach: SV leak and TV leak are merged into
     * "valve_leak" for kNN voting (geometrically indistinguishable at
     * k=3), then split back using svTransition > tvTransition heuristic.
    ...
    function classifyKNN(fVec) {
        ...
        // (SV and TV leak cards are geometrically too similar for kNN to separate)
        var votes = {};
        var totalWeight = 0;
        for (var i = 0; i < k && i < dists.length; i++) {
            var w = 1.0 / (dists[i].dist + 0.01);
            var lbl = dists[i].label;
            if (lbl === 'sv_leak' || lbl === 'tv_leak') lbl = 'valve_leak';
        ...
        // svTrans > tvTrans → SV leak (load drops gradually at top)
        // tvTrans > svTrans → TV leak (load picks up gradually at bottom)
        if (bestLabel === 'valve_leak') {
            bestLabel = (fVec[4] > fVec[5]) ? 'sv_leak' : 'tv_leak';
        }
    ...
    // L1 broad-class centroids — fleet-retrained from 55 XSPOC-classified wells
    // Used as fallback reference. Primary classifier is kNN (see KNN_TRAINING below).
    // Updated 2026-04-10.
    var CENTROIDS = {
        full_pump:    { mean: [0.789,0.617,0.683,0.167,0.167,1.000,0.680,0.754,0.090,0.743,0.139,-0.160,0.359], std: [0.070,0.180,0.186,0.050,1.863,0.050,0.237,0.209,0.050,0.277,0.092,0.093,0.247], n: 6 },
        pump_issue:   { mean: [0.763,0.640,0.729,0.148,0.805,0.933,0.518,0.679,0.066,0.821,0.141,-0.117,0.287], std: [0.188,0.219,0.176,0.050,0.295,0.190,0.398,0.292,0.050,0.254,0.104,0.079,0.167], n: 9 },
        ...
        under_filled: { mean: [0.622,0.592,0.459,0.208,0.419,0.693,0.315,0.403,0.218,0.579,0.321,-0.208,0.615], std: [0.216,0.259,0.175,0.090,0.631,0.391,0.325,0.314,0.184,0.338,0.211,0.175,0.218], n: 23 },
    };
    ...
    // 5-class hybrid: sv+tv merged to valve_leak for voting, split back via svTrans heuristic.
    // 73% 5-class LOO-CV, 71% 6-class equivalent.
    var KNN_TRAINING = [
        [0,0.6586,0.2269,0.8795,0.1292,1.0000,1.0000,0.4556,1.0000,0.0811,0.4958,0.0964,-0.0831,0.1863,0.0680],
        [0,0.8434,0.6518,0.9111,0.1435,1.0000,1.0000,0.4341,0.5259,0.0401,0.8929,0.0333,-0.0530,0.1406,0.0354],
        [0,0.8293,0.7222,0.5789,0.1600,1.0000,1.0000,0.5338,0.6859,0.0690,0.8750,0.0877,-0.1317,0.2108,0.0318],
        [0,0.7680,0.6333,0.6339,0.1892,-3.9999,1.0000,1.0000,1.0000,0.1664,0.2444,0.3214,-0.3390,0.8431,0.0189],
        [0,0.8720,0.7670,0.7273,0.1355,1.0000,1.0000,0.6580,0.4750,0.0376,0.9515,0.1212,-0.1614,0.2645,0.0353],
        ...
        [1,0.6447,0.9101,0.4602,0.1863,0.2502,0.4939,0.0685,0.1325,0.1774,1.0000,0.3186,-0.4123,0.7840,0.0313],
        ...
        [1,0.8407,0.7500,0.6132,0.2000,1.0000,1.0000,0.7463,1.0000,0.0772,1.0000,0.2264,-0.3178,0.6207,0.0645],
        ...
        [1,0.7055,0.4835,0.5634,0.2047,1.0000,1.0000,0.3676,0.2955,0.1581,0.2418,0.3521,-0.2143,0.5825,0.0652],
        ...
        [2,0.7745,0.6964,0.5870,0.2185,0.6686,1.0000,0.0656,0.8823,0.1248,0.8929,0.0217,-0.0247,0.1815,0.0258],
        ...
        [2,0.8566,0.6102,0.9881,0.0934,0.2280,1.0000,0.0816,0.5071,0.0529,0.8475,0.0119,-0.0105,0.0869,0.0040],
        [2,0.8385,0.5397,1.0000,0.1816,0.3511,1.0000,0.1124,1.0000,0.0232,0.4921,0.0256,-0.0217,0.0419,0.0107],
